...
# session that operated in it. Emits a table sorted by size with a suggested bucket. Never
...
# Transcripts dir: ~/.claude/projects/<slug>, where Claude Code builds the slug
# by replacing every non-alphanumeric character in the repo path with "-".

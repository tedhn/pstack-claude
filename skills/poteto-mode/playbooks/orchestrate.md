...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through the Agent tool. State lives in the plain TSV and JSON files of the store below, edited directly at drain points, one edit in and one line out. The upstream `orch` CLI is not part of this port, and the files were designed to stay readable without it.
...
- **Worker / verifier.** Always `isolation: "worktree"`, or `isolation: "remote"` when the account offers cloud sandboxes and the task needs nothing from this machine. Tasks that need this machine: driving the control surface (Browser tools, simulators), reading local transcripts, local IDE state, auth that exists only here. Remote workers cannot read the local store, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model from its worker.
...
Create `~/.claude/pstack/orchestrate/<project-slug>/`. Every file has exactly one writer. Owners publish facts, readers aggregate at read time. The files are plain TSV and JSON. Edit them directly.
...
- `inbox/` holds completion pointers, one small file per completion. `gates.md` parks human gates (question, options, default on no answer).
...
VERIFY       exact commands or the control-surface steps, plus known gotchas
...
2. **Install the runtime.** Create the store directory with a header-only `units.tsv` and `ledger.tsv`, an empty `inbox/`, and `gates.md`. Open the trail via the show-me-your-work skill, write the standing orders before any spawn, and seed `frontier.json` from existing PRs with `gh pr list --json number,baseRefName,headRefName,headRefOid,state`.
...
- On a completion notification, write one pointer file under `inbox/` (agent, unit, status, report path) and return to what you were doing. Never deep-review inline. A completion that needs review becomes a verifier unit. Never review a diff inside a drain.
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it via the `/loop` skill, with a long heartbeat fallback), and before a human report. Begin each batch by reading and clearing `inbox/`. Arrivals during a drain wait for the next one.
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` from `gh pr list --json number,baseRefName,headRefName,headRefOid,state` after every merge and stack mutation, since base refs drift mid-restack: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. A checkout that cannot see the PRs reports none and the recompute fails loudly rather than guessing.
- Exactly one stacker per stack may rebase or retarget, serialized within its stack. Record the holder in the standing orders. Restacks run in the stacker's own worktree, never in a worker's.
- Workers never rebase and never retarget. Babysitters follow `playbooks/babysit.md`, one per stack, scoped to one immutable frontier generation. They report conflicts to the stacker rather than restacking.
...
Append ledger rows to `ledger.tsv`, one row per verdict, keyed by PR number plus head SHA: `live-ui-verified | unit-test-verified | type-check-only | verifier-blocked | verifier-failed`. Check the current PR and head SHA against it before landing. CI green is an input to a verdict, not a verdict. Behavioral work needs better than `type-check-only`. `verifier-blocked` is not a pass. Respawn when the environment heals. `verifier-failed` gets a fix unit, not a re-verify. A worker may self-report. A verifier overrides it on the same key. A new head SHA voids the row, so re-verify after restack. The ledger answers "was this verified", not memory and not the transcript.

A unit is not done until its output is externalized the moment it lands, never batched to the end of the run. A worker pushes its branch, a verifier writes its ledger row, receipts land in the store. Work that exists only in one worktree when that worktree dies was never done.

#### Liveness and failure

- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the agent's task output. Transcript mtime is not liveness.
...
- After a Claude Code restart: in-process agents are dead, pushed branches and remote sandboxes are not. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach work by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. Nothing locks the store, so re-read it and continue.

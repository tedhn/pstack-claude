...
5. Verify on the matching surface. For a visual decision, screenshot each variant through the control surface (Browser tools or Playwright) and drive the interaction. For a behavioral or timing decision, observe the thing you are deciding by logging the timing, printing the output, or watching the render. The observation is the test here, not an assertion.

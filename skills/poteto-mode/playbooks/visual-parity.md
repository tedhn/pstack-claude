...
4. Verify each component against its baseline via image diff on the matching control surface (poteto-mode Non-negotiables). A nonzero diff is a fail. Investigate the pixel delta. `/loop` per component until the diff is zero.

...
1. Locate the prior trail. A local transcript (this session's path comes from `node ~/.claude/skills/pstack/scripts/session.js transcript`; a prior session's file lives under `~/.claude/projects/<cwd slug>/<session id>.jsonl`, and `claude --resume` lists them. Do not glob across other projects' directories, that reads private chats from unrelated work), a resumed session, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).

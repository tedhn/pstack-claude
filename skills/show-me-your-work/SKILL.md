...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript. `node ~/.claude/skills/pstack/scripts/session.js transcript` prints its path, recorded by the pstack hooks. Don't glob across `~/.claude/projects/*/`. That reads unrelated private chats. Walk the log against what actually happened:
...
Before handing back, spawn a read-only subagent (`subagent_type: Explore`) on a different model from the one that did the work, picked from `~/.claude/pstack-models.md` (`arena cross-judge pool` is a fine source). Self-review is not a substitute. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.

...
- **Drive:** how can an agent interact with it programmatically? Existing harnesses first — Playwright/Cypress specs, expect scripts, PTY helpers, curl-able endpoints, a debug port. Only then pick a generic recipe: a Playwright script (the `playwright` plugin when installed) or the Claude Browser tools for web and Electron, a tmux/PTY harness for CLI/TUI, plain HTTP for services, the iOS Simulator tool for iOS. Prefer a recipe the next agent can run from Bash with no plugin present, and name the tool-based alternative beside it.

...
Before spawning investigators, list the MCP servers available in this session. Their tools are named `mcp__<server>__<tool>`, and deferred ones show up by name in the tool list even before their schemas load. Group tools by `<server>`.
...
- `subagent_type`: `Explore` (read-only, keeps every MCP tool, so MCP-backed investigators work)
...
- `subagent_type`: `Explore` (read-only, keeps MCP tools so its quality check can spot-verify citations)

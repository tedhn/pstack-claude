...
- `Skill` tool invocations (`pstack:<skill>` or any other skill name), and `Read` tool calls against any `SKILL.md` file (project `.claude/skills/`, user-level `~/.claude/skills/`, or plugin paths under `~/.claude/plugins/`)
- `Agent` prompts that name a skill or a skill path
